...
while True:
    ...
    while True:
        m = input("\n請輸入西元前及後：")
        if m =="前" or m =="後":
            BCAD = m.replace("前","前").replace("後","")
        else:
            print("輸入錯誤!\n")
            continue
        while True:    
            n = input("請輸入哪一年：")
            pattern = re.compile(r"\d+?$") #正整數的正則
            ...
            for Webpage in range(y,y+1):
                    ...
                    response = requests.get("https://zh.wikipedia.org/wiki/" +str(BCAD) + str(Webpage) + "年")
                    ...
                    b = abc1.find(word2)
                    abc2 = abc1[a+8:b]
                    abc3 = f"西元{str(BCAD)}{str(Webpage)}年大事記：\n"
                    abc4 = "========================================\n"
